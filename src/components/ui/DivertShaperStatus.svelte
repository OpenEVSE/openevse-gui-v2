...
<div class="mt-2 mb-0 ml-1 is-flex is-flex-direction-row is-justify-content-left is-align-items-center is-flex-wrap-wrap is-size-6 has-text-weight-bold">
	...
	<span class="is-size-7 mr-2 has-text-weight-bold has-text-dark">{$_("status-shaper-houseload")}: <span class="has-text-info">{$status_store.shaper_live_pwr} W</span></span>
	<span class="is-size-7 mr-2 has-text-weight-bold has-text-dark">{$_("status-current-available")}: <span class="{$status_store.shaper_cur < 6?"has-text-danger":"has-text-primary"}">{$status_store.shaper_cur} A</span></span>
</div>
